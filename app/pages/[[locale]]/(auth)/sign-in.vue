    ...
    <div class="absolute bottom-1/2 left-1/2 -translate-x-1/2 translate-y-1/2 w-full text-center p-4">
      ...
      <p class="mt-4 text-sm text-black">
        <T text="By signing in, you agree to our {terms} and {privacy}." :values="{ terms: 'true', privacy: 'true' }">
          <template #terms>
            <A href="/legal/terms" class="text-brown-500 hover:text-black hover:underline focus:outline-none border-y-2 border-transparent focus:border-b-blue-500 w-fit text-sm mr-1">{{ t('Terms of Service') }}</A>
          </template>
          <template #privacy>
            <A href="/legal/privacy" class="text-brown-500 hover:text-black hover:underline focus:outline-none border-y-2 border-transparent focus:border-b-blue-500 w-fit text-sm">{{ t('Privacy Policy') }}</A>
          </template>
        </T>
      </p>
    </div>
